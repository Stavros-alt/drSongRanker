document.addEventListener("DOMContentLoaded", () => {
	...
	const preventDuplicatesToggle = document.getElementById(
		"prevent-duplicates-toggle",
	);
	const showGlobalDiffToggle = document.getElementById(
		"show-global-diff-toggle",
	);
	...
	const hideMatchupRankingsToggle = document.getElementById(
		"hide-matchup-rankings-toggle",
	);
	const hideAccuracyToggle = document.getElementById("hide-accuracy-toggle");
	const hideAccuracyPercentToggle = document.getElementById(
		"hide-accuracy-percent-toggle",
	); // discord anon wanted this
	...
	const finishShowRankingsBtn = document.getElementById(
		"finish-show-rankings-btn",
	);
	...
	const customChecklistContainer = document.getElementById(
	...
	function loadBackgroundColor() {
		...
		if (savedBg) {
			...
			if (
				customBgLabel &&
				!Array.from(bgBtns).some((b) => b.dataset.bg === savedBg)
			) {
	...
	function exportData() {
		const allKeys = [
			...
			"utySongRankerState",
			"tsusSongRankerState",
			"combinedSongRankerState",
			"drSongRankerGlobalState",
			"drSongRankerVolume",
			"drSongRankerCustomLists",
			"drSongRankerTheme",
			"drSongRankerBackground",
	...
	function importData(file) {
		...
		reader.onload = (e) => {
			try {
				...
				if (
					!confirm(
						"This will OVERWRITE all current ranking data. make sure you exported first. continue?",
					)
				) {
					return;
				}
	...
	const MERGE_KEY_TO_GAME = Object.fromEntries(
		Object.entries(MERGE_STATE_KEYS).map(([g, k]) => [k, g]),
	);

	// song id ranges per game so we can map orphaned songs
	const MERGE_GAME_RANGES = [
		{ game: "deltarune", min: 1, max: 999 },
		{ game: "undertale", min: 1000, max: 1999 },
		{ game: "uty", min: 2000, max: 3999 },
		{ game: "tsus", min: 4000, max: 9999 },
	];

	function detectGameFromSongId(id) {
		for (const r of MERGE_GAME_RANGES) {
			if (id >= r.min && id <= r.max) return r.game;
		}
		return "deltarune";
	}
	...
	function rebuildMergedResults() {
		...
		for (let fi = 0; fi < mergedFiles.length; fi++) {
			...
			for (const [game, key] of Object.entries(MERGE_STATE_KEYS)) {
				...
				if (!perGame[game]) perGame[game] = {};
				if (!gameDedup[game]) gameDedup[game] = new Set();

				for (const s of songs) {
					const comparisons = s.felfebComparisons || s.comparisons || 0;
					if (comparisons === 0) continue; // didn't actually rate this one
					...
					perGame[game][s.id].ratings.push(rating);
					perGame[game][s.id].voterNames.push(file.name);
				}
			}
		}

		// route songs from combined key into their native game buckets unless already there
		for (let fi = 0; fi < mergedFiles.length; fi++) {
			const file = mergedFiles[fi];
			for (const [, key] of Object.entries(MERGE_STATE_KEYS)) {
				const raw = file.data[key];
				if (!raw) continue;
				let parsed;
				try {
				...
				const songs = parsed.songs || [];
				for (const s of songs) {
					const nativeGame = detectGameFromSongId(s.id);
					if (nativeGame === (MERGE_KEY_TO_GAME[key] || "")) continue; // already in main loop
		...
		mergedResults = perGame;
	}

	function renderGroupMergedResults(selectedGame) {
		const listEl = document.getElementById("merge-ranking-list");
		const resultsEl = document.getElementById("merge-results");
		const personCountEl = document.getElementById("merge-person-count");

		if (!listEl || !resultsEl) return;

		const gameData = mergedResults[selectedGame] || {};
		const entries = Object.entries(gameData);

		if (entries.length === 0) {
			resultsEl.style.display = "none";
			if (personCountEl) personCountEl.textContent = "";
			return;
		}
		...
		const sorted = entries
			.map(([id, d]) => ({
				...
				voterCount: d.ratings.length,
				voterNames: d.voterNames,
				ratings: d.ratings,
			}))
		...
		for (let i = 0; i < sorted.length; i++) {
			...
			li.style.cssText =
				"display: flex; align-items: center; justify-content: space-between; padding: 6px 4px; border-bottom: 1px solid #222; gap: 8px;";

			const rankSpan = document.createElement("span");
			rankSpan.style.cssText =
				"color: #888; font-size: 0.85em; min-width: 28px; text-align: right;";
			rankSpan.textContent = `#${i + 1}`;
			li.appendChild(rankSpan);

			const nameSpan = document.createElement("span");
			nameSpan.style.cssText =
				"flex: 1; text-align: left; font-size: 0.9em; color: #fff;";
			nameSpan.textContent = s.name;
			nameSpan.title = s.voterNames
				.map((n, idx) => `${n}: ${Math.round(s.ratings[idx])}`)
				.join("\n");
			li.appendChild(nameSpan);

			const countBadge = document.createElement("span");
			countBadge.style.cssText =
				"color: #888; font-size: 0.75em; min-width: 18px; text-align: center;";
			countBadge.textContent = `${s.voterCount}v`;
			countBadge.title = s.voterNames.join("\n");
			li.appendChild(countBadge);

			const ratingSpan = document.createElement("span");
			ratingSpan.style.cssText =
				"color: var(--accent-color); font-size: 0.9em; min-width: 50px; text-align: right;";
			ratingSpan.textContent = Math.round(s.avgRating);
		...
		for (const file of files) {
			...
			reader.onload = (e) => {
				try {
					...
					if (!data.version) {
						pending--;
						if (pending === 0) afterMergeLoad();
						return; // invalid, skip silently
					}
					mergedFiles.push({ name: file.name, data });
				} catch (err) {
					// corrupt file, skip
				}
				pending--;
	...
	// wire up merge UI
	const groupMergeBtn = document.getElementById("group-merge-btn");
	const groupMergeModal = document.getElementById("group-merge-modal");
	const closeMergeBtn = document.getElementById("close-merge-btn");
	const mergeAddFilesBtn = document.getElementById("merge-add-files-btn");
	const mergeFileInput = document.getElementById("merge-file-input");
	const mergeClearBtn = document.getElementById("merge-clear-btn");
	const mergeGameSelect = document.getElementById("merge-game-select");

	if (groupMergeBtn && groupMergeModal) {
	...
	if (closeMergeBtn && groupMergeModal) {
		closeMergeBtn.addEventListener("click", () => {
			groupMergeModal.style.display = "none";
		});
		// close on overlay click
		groupMergeModal.addEventListener("click", (e) => {
			if (e.target === groupMergeModal) groupMergeModal.style.display = "none";
		});
	}

	if (mergeAddFilesBtn && mergeFileInput) {
		mergeAddFilesBtn.addEventListener("click", () => mergeFileInput.click());
	}
	...
	if (mergeGameSelect) {
		mergeGameSelect.addEventListener("change", (e) => {
			renderGroupMergedResults(e.target.value);
		});
	}
	...
	function loadState() {
		...
		if (saved) {
			...
			if (
				Object.keys(state.customLists).length === 0 &&
				parsed.customLists &&
				Object.keys(parsed.customLists).length > 0
			) {
			...
			state.songs = sourceList.map((baseSong) => {
				const possibleStates = [
					parsed && parsed.songs
						? parsed.songs.find((s) => s.id === baseSong.id)
						: null,
					drParsed && drParsed.songs
						? drParsed.songs.find((s) => s.id === baseSong.id)
						: null,
					utParsed && utParsed.songs
						? utParsed.songs.find((s) => s.id === baseSong.id)
						: null,
					utyParsed && utyParsed.songs
						? utyParsed.songs.find((s) => s.id === baseSong.id)
						: null,
					tsusParsed && tsusParsed.songs
						? tsusParsed.songs.find((s) => s.id === baseSong.id)
						: null,
					combinedParsed && combinedParsed.songs
						? combinedParsed.songs.find((s) => s.id === baseSong.id)
						: null,
				].filter((s) => s !== null);

				const freshest = possibleStates.sort(
					(a, b) => (b.comparisons || 0) - (a.comparisons || 0),
				)[0];
		...
		if (
			!validLists.includes(state.activeRankerList) &&
			!state.customLists[state.activeRankerList]
		) {
			state.activeRankerList = "all";
		...
		// force duplicates prevention on for legacy saves
	...
	function updateGameUI() {
		if (state.currentGame === "deltarune") {
			...
			if (mainTitle)
				mainTitle.textContent = "Which Undertale Yellow song is better?";
		} else {
			if (drToggle) drToggle.classList.remove("active");
			if (combinedToggle) combinedToggle.classList.remove("active");
			if (utToggle) utToggle.classList.remove("active");
		...
		if (drToggle && state.currentGame !== "deltarune")
			drToggle.classList.remove("active");
		if (utToggle && state.currentGame !== "undertale")
			utToggle.classList.remove("active");
		if (utyToggle && state.currentGame !== "uty")
			utyToggle.classList.remove("active");
		if (tsusToggle && state.currentGame !== "tsus")
			tsusToggle.classList.remove("active");
		if (combinedToggle && state.currentGame !== "combined")
			combinedToggle.classList.remove("active");

		if (combinedToggle) {
			combinedToggle.style.display = globalState.combinedDiscovered
				? "inline-block"
				: "none";
		}
	}
	...
	function checkSecretsGlobal() {
		const unlocked =
			localStorage.getItem("drSongRankerSecretsUnlocked") === "true";
	...
	if (playerProgress) {
		const handleSeek = (e) => {
			if (
				!currentActiveAudio ||
				!currentActiveAudio.duration ||
				currentActiveAudio.duration === Infinity
			)
				return;
	...
	if (createListBtn) {
		createListBtn.addEventListener("click", () => {
			...
			if (
				...
				["all", "all_plus", "hidden", "1", "2", "3", "4"].includes(name) ||
				name.startsWith("duration_")
			) {
	...
	saveCustomBtn.addEventListener("click", () => {
		...
		const checked = Array.from(
			customChecklistContainer.querySelectorAll("input:checked"),
	...
	suggestBtn.addEventListener("click", () => {
		window.open(
			"https://stavros-alt.github.io/Stavros-alt/suggestions.html",
			"_blank",
		);
	});
	...
	function getMatchHistory(song) {
		return isFelfebRanker()
			? song.felfebMatchHistory || []
			: song.matchHistory || [];
	}
	...
	function getFilteredSongs() {
		...
		if (filter === "all" || filter === "combined_all") {
			...
			return pool.filter(
				(s) =>
					(!s.hidden || state.secretsUnlocked) &&
					getChaptersForSong(s).includes(filter),
			);
			...
			return pool.filter(
				(s) => s.felfebFile && (!s.hidden || state.secretsUnlocked),
			);
			...
			return pool.filter(
				(s) => ids.includes(s.id) && (!s.hidden || state.secretsUnlocked),
			);
		}
	...
	function presentNewPair() {
		...
		do {
			...
			if (boostedSong && Math.random() < 0.25) {
				...
				// 60% for underrated
				const sortedByVotes = [...availableSongs].sort(
					(a, b) => getComparisons(a) - getComparisons(b),
				);
				const uncertaintyPoolSize = Math.max(
					5,
			...
			const sortedOpponents = [...availableSongs]
				...
				.sort(
					(a, b) =>
						Math.abs(getRating(a) - getRating(song1)) -
						Math.abs(getRating(b) - getRating(song1)),
				);
			...
			// check match history to avoid repeats
	...
	async function fetchMatchupStats(songA, songB) {
		...
		try {
			const rpcName = isFelfebRanker()
				? "get_felfeb_global_matchups"
				: "get_song_agreement_stats";
			console.log(
				`[Agreement] Fetching stats for target=${songA.id} (${songA.name}) against opponent=${songB.id} (${songB.name})`,
			);

			const { data, error } = await supabaseClient.rpc(rpcName, {
				target_song_id: parseInt(songA.id),
			});
			...
			console.log(
				`[Agreement] Match not found for song A as target. Trying song B: ${songB.id}`,
			);
			const { data: dataB, error: errorB } = await supabaseClient.rpc(rpcName, {
				target_song_id: parseInt(songB.id),
			});
			if (!errorB && dataB && Array.isArray(dataB)) {
				...
				if (matchupB) {
					...
					return {
						wins: parseInt(matchupB.losses),
						losses: parseInt(matchupB.wins),
					};
				}
			}
		} catch (err) {
	...
	async function handleChoice(winner) {
		...
		// only fetch total votes every 15 votes to save on db costs
	...
	function playPreview(songKey) {
		...
		audioEl.onerror = () =>
			console.error("preview audio load error for:", previewFile);
	...
	function playFullSong(songKey) {
		...
		audioEl.onerror = () =>
			console.error("full song audio load error for:", fullFile);
		...
		audioEl
			...
			.catch((err) => {
				console.error("Full playback failed:", err.message);
			});
	}
	...
	async function recordCommunityVote(winnerId, loserId) {
		...
		try {
			...
			console.error(
				`Error recording community vote for ${state.currentGame}:`,
				error.message,
			);
		}
	}
			...
			try {
				...
				cachedCommunitySongs = combinedData.map((cSong) => {
					...
					return {
						...
						duration: localSong ? localSong.duration : 0,
						felfebFile: localSong ? localSong.felfebFile : undefined,
	...
	async function displayCommunityRankings() {
		...
		try {
			...
			filteredSongs.forEach((song, index) => {
				...
				li.addEventListener("click", () => playSongFromCurrentList(index));
	...
	async function showGlobalMatchHistory(song) {
		...
		historyList.innerHTML =
			'<p style="color: #666; text-align: center;">Fetching global matchup data...</p>';
		...
		try {
			const rpcName = isFelfebRanker()
				...
				: "get_song_agreement_stats";
			const { data, error } = await supabaseClient.rpc(rpcName, {
				target_song_id: parseInt(song.id),
			});
			if (error) throw error;

			if (!data || data.length === 0) {
				historyList.innerHTML =
					'<p style="color: #666; text-align: center;">No global matchups recorded yet.</p>';
				return;
			}
			...
			const allSongsMeta = [...state.songs];
			if (state.currentGame === "combined") {
	...
	function displayRankings() {
		...
		const hasFelfebGlobal =
			state.felfebMode &&
			(state.currentGame === "deltarune" || state.currentGame === "undertale");
		...
		if (
			state.showGlobalDiff &&
			cachedCommunityGame === state.currentGame &&
			cachedCommunitySongs.length > 0
		) {
			const filteredGlobal = filterSongsByChapter(
				cachedCommunitySongs,
				currentChapterFilter,
			);
		...
		sortedSongs.forEach((song, index) => {
			...
			if (globalRanks !== null && globalRanks[song.id] !== undefined) {
				const globalPos = globalRanks[song.id];
				const rankDiff = globalPos - index;
				if (rankDiff !== 0) {
	...
	function filterSongsByActiveList(songs) {
		...
		if (activeList === "hidden") {
			return songs.filter(
				(s) =>
					s.hidden ||
					(state.currentGame === "deltarune" && s.id > 200) ||
					(state.currentGame === "undertale_yellow" && s.id > 3000),
			);
		}

		if (activeList === "custom") {
			const customSelection = JSON.parse(
				localStorage.getItem("drSongRankerCustomSelection") || "[]",
			);
	...
	function showMatchHistory(song) {
		...
		if (history.length === 0) {
			historyList.innerHTML =
				'<p style="color: #666; text-align: center;">No matches recorded yet.</p>';
		} else {
			history.forEach((match, index) => {
				...
				if (swapBtn) {
					swapBtn.addEventListener("click", (e) => {
						...
						if (
							confirm(
								`Swap the result of the match against ${match.opponent}? Ratings will be adjusted.`,
							)
						) {
	...
	function swapMatchResult(song, index) {
		...
		const opponentMatchIndex = opponentHistoryList.findIndex(
			(m) => m.time === match.time && m.opponentId === song.id,
		);
		...
		match.result =
			oldResult === "win" ? "loss" : oldResult === "loss" ? "win" : "tie";
		opponentMatch.result =
			oldResult === "win" ? "win" : oldResult === "loss" ? "loss" : "tie";
	...
	function undoVote() {
		...
		if (songA && songB) {
			...
			const historyListA = state.history.wasFelfeb
				? songA.felfebMatchHistory || []
				: songA.matchHistory || [];
			const historyListB = state.history.wasFelfeb
				? songB.felfebMatchHistory || []
				: songB.matchHistory || [];
	...
	function updateMainFilterOptions() {
		...
		const allOptionValues = Array.from(mainFilterSelect.options).map(
			(o) => o.value,
		);
	...
	if (mainFilterSelect) {
		mainFilterSelect.addEventListener("change", (e) => {
			...
			if (editListBtn) {
				editListBtn.style.display = state.customLists[currentChapterFilter]
					? "inline-block"
					: "none";
			}
	...
	if (applyMixBtn) {
		applyMixBtn.addEventListener("click", () => {
			...
			let option = Array.from(mainFilterSelect.options).find(
				(opt) => opt.value === mixKey,
			);
			if (!option) {
				...
				const mixChaptersOpt = mainFilterSelect.querySelector(
	...
	function getExportSourceSongs() {
		...
		const pool = isGlobal ? cachedCommunitySongs : state.songs;
		...
		const filtered = filterSongsByChapter(pool, currentChapterFilter);
	...
	exportZipBtn.addEventListener("click", async () => {
		...
		try {
			...
			if (addedCount === 0) {
				alert(
					"Failed to fetch any MP3 files. Are you running this on a server? Check console for CORS or 404s.",
				);
				return;
			}
	...
	exportTextBtn.addEventListener("click", () => {
		...
		const textList = exportPool
			.map((s) => {
				...
				else if (state.currentGame === "undertale_yellow")
					ostName = "Undertale Yellow OST";
		...
		navigator.clipboard
			...
			.then(() => {
				alert(
					"Copied to clipboard. Paste it into Spotify/YouTube tools and leave me alone.",
				);
				exportModal.style.display = "none";
			})
			.catch((err) => {
